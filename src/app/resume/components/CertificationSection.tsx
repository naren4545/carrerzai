  ...
  <div className="flex items-start justify-between bg-white rounded-lg p-4 mb-4">
    <div className="flex items-start gap-4">
      ...
      <div>
        ...
        <a href={certification.url} target="_blank" rel="noopener noreferrer" className="text-blue-500">
          View Certification
        </a>
      </div>
    </div>
    <div className="flex items-center space-x-2">
      <button type="button" className="text-gray-500" onClick={() => onEdit(certification)}>
        <Image src={editPen} alt="Edit Icon" className="w-[20px]" />
      </button>
      <button type="button" className="text-red-500" onClick={() => onDelete(certification._id)}>
...
const CertificationSection: React.FC<CertificationSectionProps> = ({ certifications }) => {
  ...
  const handleSave = (cert: Certification) => {
    if (currentCert) {
      setCertList((prev) =>
        prev.map((c) => (c._id === currentCert._id ? { ...cert, _id: currentCert._id } : c))
      );
    } else {
      setCertList((prev) => [...prev, { ...cert, _id: Date.now().toString() }]);
    }
    setIsFormOpen(false);
  };
  ...
  return (
    <div className="p-6">
      ...
      {certList.map((cert) => (
        <CertificationCard key={cert._id} certification={cert} onEdit={handleEdit} onDelete={handleDelete} />
      ))}
      {isFormOpen && (
        <CertificationForm onClose={() => setIsFormOpen(false)} onSave={handleSave} initialData={currentCert} />
      )}
    </div>
  );
};
...
const CertificationForm: React.FC<CertificationFormProps> = ({ initialData, onSave, onClose }) => {
...
<div onClick={handleBackdropClick} className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
      <div className="bg-white p-6 rounded-lg shadow-lg w-full max-h-fit h-full overflow-auto max-w-[1092px]" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
