 ...
 return (
  <div className="flex items-start justify-between bg-white  rounded-lg p-4 mb-4 ">
    <div className="flex items-start gap-4">
      <div>
      <Image src={educationImg} alt="Education Icon" className="h-12 w-12 pb-2" />
      <Image src={educationlinefrom} alt="Education Icon" className="h-12 w-12" />
      </div>
      <div>
        ...
        <div className="flex gap-2 mt-2">
          ...
          {startYear} - {completionYear}                   </span>
        </div>
      </div>
    </div>
    <div className="flex items-center space-x-2">
      <button type="button" className="text-gray-500 cursor-pointer" onClick={() => onEdit(education)} >
        <Image src={editPen} alt="Edit Icon" className="md:w-auto w-[20px]" />
      </button>
      <button type="button" className="text-red-500 cursor-pointer" onClick={() => onDelete(education._id)} >
      <Image src={deleteIcon} alt="delete Icon" className="md:w-auto w-[20px]" />
      </button>
    </div>
  </div>
);
  ...
  const handleSave = (education: Education) => {
    if (currentEducation) {
      setEducationList((prev) =>
        prev.map((edu) => (edu._id === currentEducation._id ? { ...education, _id: currentEducation._id } : edu))
      );
    } else {
      setEducationList((prev) => [...prev, { ...education, _id: Date.now().toString() }]);
    }
    setIsFormOpen(false);
  };
...
const EducationForm: React.FC<EducationFormProps> = ({ initialData, onSave, onClose }) => {
...
<div onClick={handleBackdropClick} className="fixed inset-0 flex items-center justify-center py-4 bg-black bg-opacity-50 z-50">
      <div className="bg-white py-6 rounded-2xl shadow-lg w-full h-full overflow-auto max-w-[1092px]" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
